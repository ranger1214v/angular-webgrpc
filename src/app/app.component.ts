...
export class AppComponent implements OnInit {
  ...
  public messageList$ = this.messageItem$.pipe(
    scan(
      (current: Item.AsObject[], newItem: Item.AsObject) => [
        ...current,
        newItem,
      ],
      []
  ...
  async ngOnInit() {
    this.name$.next(
      localStorage.getItem('name') ?? prompt('請輸入你的名字') ?? '沒有名字'
    );
  ...
  btnSendMessage(message: string) {
    ...
    this.clientWeb.unaryAddItem(item, (err, response) =>
